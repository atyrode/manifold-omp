...
await runSdkScenario(async (ctx: SdkScenarioContext) => {
  ...
  let storage = await NativeBrokerStorage.create(dbPath, { refreshOAuthCredential: ctx.refreshOAuthCredential });
  ...
  storage = await NativeBrokerStorage.create(join(ctx.root, "authority.db"), { refreshOAuthCredential: ctx.refreshOAuthCredential });
  broker = undefined;
  remote = undefined;
  try {
