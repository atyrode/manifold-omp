...
import { DEFAULT_SERVER_IDLE_TIMEOUT_S } from "@oh-my-pi/pi-ai/auth-broker/types";
...
export function startNativeBroker(options: NativeBrokerOptions): NativeBrokerHandle {
  ...
  const pendingForwards = new Set<Promise<Response>>();
  ...
  const quiesce = (): Promise<void> => {
    ...
    // Stop the SDK's producers immediately while retaining its admitted writes.
    // Authentication here is not data-operation admission. A forward not yet
    // admitted by the SDK receives 503 without mutation; SDK-admitted writes
    // finish before quiesce. Retire both kinds before reporting drained.
    const stockDrain = upstream.quiesce().then(() => false, () => true);
    draining = (async () => {
      await Promise.allSettled(pendingForwards);
      let failed = await stockDrain;
      storage.stopRefreshAdmission();
      try { await upstream.close(); } catch { failed = true; }
      try { await storage.drainRefreshes(); } catch { failed = true; }
      if (failed) throw new Error("Broker drain failed");
      state = "drained";
    })();
    return draining;
  };
  ...
  const forward = async (request: Request, url: URL): Promise<Response> => {
    ...
    headers.delete("host");
    // Once buffered, caller disconnect must not abort SDK-admitted work.
  ...
  const server = Bun.serve({
    ...
    async fetch(request): Promise<Response> {
      ...
      finally { pendingForwards.delete(operation); }
    },
  });
  try {
